...
<div class="min-h-screen px-4 py-16">
    <div class="mx-auto max-w-lg">
        ...
        <div class="mb-6 h-3 rounded-full bg-white/60 backdrop-blur-sm overflow-hidden">
            <div class="h-3 rounded-full bg-gradient-to-r from-ocean-400 to-candy-400 transition-all duration-500 ease-out"
                style="width: {{ (($currentIndex + ($answered ? 1 : 0)) / $session->question_count) * 100 }}%"></div>
        </div>

        {{-- Question card --}}
        <div class="rounded-2xl bg-white/80 backdrop-blur-sm border-2 border-white p-6" x-data="{ showQuestion: true }"
            x-init="showQuestion = true" wire:key="question-card-{{ $currentIndex }}">
            <h2 class="text-lg font-bold text-gray-800 leading-snug animate-fade-in"
            ...
            <div class="mt-6 grid gap-3">
                @foreach ($this->question->choices as $choiceIndex => $choice)
                    <button wire:key="choice-{{ $choice->id }}" wire:click="selectAnswer({{ $choice->id }})"
                        @disabled($answered) x-data="{ pressed: false }"
                        x-on:click="if (!{{ $answered ? 'true' : 'false' }}) { pressed = true; setTimeout(() => pressed = false, 200) }"
                        :class="pressed ? 'scale-[0.97]' : 'scale-100'" @class([
                            ...
                            'border-mint-400 bg-mint-50 text-mint-600 animate-pop' =>
                                $answered && $choice->is_correct,
                            'border-candy-400 bg-candy-50 text-candy-600 animate-wiggle' =>
                                $answered && $selectedChoiceId === $choice->id && !$choice->is_correct,
                            'border-gray-100 bg-gray-50 opacity-40' =>
                                $answered && $selectedChoiceId !== $choice->id && !$choice->is_correct,
                        ])
                        style="animation-delay: {{ $choiceIndex * 0.05 }}s">
                        <span
                            class="mr-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-gray-100 text-sm font-bold text-gray-400">{{ $choice->label }}</span>
                        {{ $choice->text }}
                    </button>
                @endforeach
            </div>
            ...
            @if ($answered)
                ...
                <button wire:click="nextQuestion" x-data="{ pressed: false }"
