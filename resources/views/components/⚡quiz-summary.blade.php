...
new #[Title('Quiz Results — Quiz App')] class extends Component {
    ...
    public function playAgain(): void
    {
        ...
        $questionIds = $movie->questions()->active()->inRandomOrder()->limit($this->session->question_count)->pluck('id')->toArray();
    ...
    {
        $percentage = $this->session->question_count > 0 ? round(($this->session->correct_count / $this->session->question_count) * 100) : 0;
...
<div class="min-h-screen px-4 py-16 flex items-center justify-center">
    <div class="mx-auto w-full max-w-lg">
        <div class="rounded-2xl bg-white/80 backdrop-blur-sm border-2 border-white p-8 text-center"
            ...
            <div x-show="shown" x-transition:enter="transition ease-out duration-500"
                ...
                <div class="mt-8">
                    <div class="text-6xl font-bold bg-gradient-to-r from-ocean-500 to-candy-500 bg-clip-text text-transparent"
                        x-text="count + '%'">
                        0%
                    </div>
                ...
                @if ($percentage >= 80)
                    <p class="mt-4 text-lg font-bold text-mint-500 animate-bounce-in"
                        style="animation-delay: 0.8s; animation-fill-mode: both">
                        Amazing job!
                    </p>
                @elseif ($percentage >= 50)
                    <p class="mt-4 text-lg font-bold text-ocean-500 animate-bounce-in"
                        style="animation-delay: 0.8s; animation-fill-mode: both">
                        Good effort!
                    </p>
                @else
                    <p class="mt-4 text-lg font-bold text-candy-500 animate-bounce-in"
                        style="animation-delay: 0.8s; animation-fill-mode: both">
                        Keep practicing!
                    </p>
                @endif
            ...
            <div class="mt-8 grid gap-3" x-show="shown" x-transition:enter="transition ease-out duration-300 delay-500"
                x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                <button wire:click="playAgain" x-data="{ pressed: false }"
                ...
                <button wire:click="shareResults" x-data="{ pressed: false }"
                    x-on:click="pressed = true; setTimeout(() => pressed = false, 300)"
                    :class="pressed ? 'scale-95' : 'scale-100'"
