...
new #[Title('Progress — Quiz App')] class extends Component {
    ...
    {
        $totals = QuizSession::whereNotNull('completed_at')->selectRaw('SUM(correct_count) as total_correct, SUM(question_count) as total_questions')->first();
    ...
    {
        return QuizSession::query()->whereNotNull('completed_at')->join('movies', 'movies.id', '=', 'quiz_sessions.movie_id')->selectRaw('movies.id, movies.title, movies.slug, COUNT(*) as attempts, MAX(quiz_sessions.correct_count * 100 / quiz_sessions.question_count) as best_score, MAX(quiz_sessions.completed_at) as last_played')->groupBy('movies.id', 'movies.title', 'movies.slug')->orderByDesc('last_played')->get();
    }
};
?>
    ...
    <div class="mx-auto max-w-lg">
        ...
        @if ($this->totalQuizzes > 0)
            <div class="mb-6 flex justify-end">
                <button wire:click="shareStats" x-data="{ pressed: false }"
        ...
        @if ($this->totalQuizzes === 0)
            <div
                class="rounded-2xl bg-white/80 backdrop-blur-sm border-2 border-white p-10 text-center animate-fade-in">
        ...
        @else
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="rounded-2xl bg-white/80 backdrop-blur-sm border-2 border-white p-5 text-center animate-fade-in-up"
                ...
                @foreach ($this->movieStats as $index => $stat)
                    <div wire:key="stat-{{ $stat->id }}"
                        ...
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500">
                                    ...
                                    &middot; Last played
                                    {{ \Carbon\Carbon::parse($stat->last_played)->diffForHumans() }}
                                </p>
                            </div>
                        ...
                        <div class="mt-3 h-2.5 rounded-full bg-gray-100 overflow-hidden">
                            <div class="h-2.5 rounded-full bg-gradient-to-r from-mint-400 to-mint-500 transition-all duration-700"
                                style="width: {{ round($stat->best_score) }}%"></div>
                        </div>
                    </div>
                @endforeach
            </div>
