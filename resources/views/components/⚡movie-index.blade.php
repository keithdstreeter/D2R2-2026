...
new #[Title('Movies — Quiz App')] class extends Component {
    ...
    public function stats(): \Illuminate\Support\Collection
    {
        ...
        return QuizSession::query()->whereIn('movie_id', $movieIds)->whereNotNull('completed_at')->selectRaw('movie_id, count(*) as attempts, max(correct_count * 100 / question_count) as best_score, max(completed_at) as last_played')->groupBy('movie_id')->get()->keyBy('movie_id');
    }
};
?>
...
<div class="min-h-screen px-4 py-16">
    <div class="mx-auto max-w-lg">
        ...
        @if ($this->hasNewContent)
            <div
                class="mb-4 flex items-center justify-between rounded-2xl bg-mint-50 border-2 border-mint-200 p-4 animate-fade-in">
                <p class="text-sm font-medium text-mint-700">New content available!</p>
                <button wire:click="dismissNewContent"
                    class="text-xs font-medium text-mint-500 hover:text-mint-700 transition-colors">Dismiss</button>
            </div>
        @endif
        ...
        <div class="grid gap-4">
            @foreach ($this->movies as $index => $movie)
                <a href="{{ route('movies.show', $movie->slug) }}" wire:key="movie-{{ $movie->id }}" wire:navigate
                    x-data="{ pressed: false }" x-on:mousedown="pressed = true" x-on:mouseup="pressed = false"
                    x-on:mouseleave="pressed = false" x-on:touchstart="pressed = true" x-on:touchend="pressed = false"
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">{{ $movie->release_year }} &middot;
                                {{ $movie->questions_count }} questions</p>
                        </div>

                        @if ($this->stats->has($movie->id))
                            <div class="text-right text-sm">
                                <div class="font-bold text-mint-600">{{ round($this->stats[$movie->id]->best_score) }}%
                                </div>
                                <div class="text-gray-400">{{ $this->stats[$movie->id]->attempts }}
                                    {{ Str::plural('attempt', $this->stats[$movie->id]->attempts) }}</div>
                            </div>
                        @else
                            <span
                                class="rounded-full bg-ocean-100 px-3 py-1 text-xs font-medium text-ocean-500">New</span>
                        @endif
                    </div>
                    ...
                    @if ($this->stats->has($movie->id))
                        <div class="mt-3">
                            <div class="h-2 rounded-full bg-gray-100">
                                <div class="h-2 rounded-full bg-gradient-to-r from-mint-400 to-mint-500 transition-all duration-500"
                                    style="width: {{ round($this->stats[$movie->id]->best_score) }}%"></div>
                            </div>
                            <p class="mt-1.5 text-xs text-gray-400">
                                Last played
                                {{ \Carbon\Carbon::parse($this->stats[$movie->id]->last_played)->diffForHumans() }}
                            </p>
                        </div>
                    @endif
                </a>
